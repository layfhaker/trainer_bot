...
from app.keyboards import (
    ...
    kb_slot_actions, kb_admin_slots_root
)
...
# ---------------- tournaments placeholder (MVP later) ----------------
@router.callback_query(F.data == "tour:list")
async def cb_tour_list(call: CallbackQuery):
    await call.message.edit_text("Турниры будут добавлены во 2-й версии.", reply_markup=kb_back("main"))
    await call.answer()
...
async def cb_admin_group_settings(call: CallbackQuery):
    ...
    text = (
        ...
        f"Закрытие записи: <b>{'в момент начала' if s['close_mode']=='at_start' else f'за {s.get('close_minutes_before')} мин.'}</b>\n\n"
...
async def cb_admin_invites(call: CallbackQuery):
    if not is_admin(call.from_user.id):
        await call.answer("Нет доступа.", show_alert=True)
        return
    await db.set_mode(call.from_user.id, "admin_invite:create")
    await call.message.edit_text(
        "Создание пригласительной ссылки.\n"
        "Отправьте ID группы числом (например 1).\n"
        "/cancel — отмена.",
...
async def message_router(message: Message):
    ...
    # invite create: expects group id
    if mode == "admin_invite:create":
