...
async def cb_admin_group_settings(call: CallbackQuery):
    ...
    close_text = (
        ...
        else f"?? {s.get('close_minutes_before')} ???."
    )
    text = (
        f"<b>Занятие</b>\n"
        f"<b>Занятие</b>\n"
...
async def cb_admin_invites(call: CallbackQuery):
    ...
    if total == 0:
        rows = [
            [__import__("aiogram").types.InlineKeyboardButton(text="? ??????? ??????", callback_data="admin:group:create")],
...
async def cb_admin_invite_pickgroup(call: CallbackQuery, page: int):
    ...
    nav = []
    if page > 0:
        nav.append(__import__("aiogram").types.InlineKeyboardButton(text="??", callback_data=f"admin:invite:pickgroup:page:{page-1}"))
    if offset + limit < total:
        nav.append(__import__("aiogram").types.InlineKeyboardButton(text="??", callback_data=f"admin:invite:pickgroup:page:{page+1}"))
    if nav:
        rows.append(nav)
